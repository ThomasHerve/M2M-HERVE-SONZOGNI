...
void ledBlink(int time) {
		...
		TimerSetValue(&DemoLedTimer,  time);
    ...
    case DEVICE_SEND:
    {
      ...
      if (SensorTimerWakeup) /*the wakeup comes from Timer*/
      {
        ...
        /*Sensor reading on slave device*/
        	ledBlink(1000);
			LoraDriverCallbacks->Callback1(&SendDataBinary);
        ...
        // Downlink data
        /*
        uint8_t ReceivedDownlinkData[4096];
        sReceivedDataString_t StructDownLinkData = { ReceivedDownlinkData, 0};
        LoraCmdRetCode = Lora_ReceivedData(&StructDownLinkData);*/
        /*if(ReceivedDownlinkData[0] != 0) {
        	ledBlink();
        }*/
